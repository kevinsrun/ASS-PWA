...
declare
  ...
  v_latest_gates_passed boolean;
begin
  if p_approved_by is distinct from p_user_id then raise exception 'Approver must match the model owner'; end if;
  select * into v_candidate from public.model_candidates where id=p_candidate_id and user_id=p_user_id for update;
  if not found then raise exception 'Candidate not found'; end if;
  if v_candidate.status not in ('candidate','evaluating','approved') then raise exception 'Candidate status % is not promotable',v_candidate.status; end if;
  if v_candidate.ollama_model_name is null or btrim(v_candidate.ollama_model_name)='' then raise exception 'Candidate has no packaged Ollama model'; end if;
  select gates_passed into v_latest_gates_passed
  from public.model_eval_runs
  where candidate_id=p_candidate_id and user_id=p_user_id and status='completed'
  order by completed_at desc nulls last,started_at desc
  limit 1;
  if v_latest_gates_passed is distinct from true then raise exception 'Latest completed candidate evaluation did not pass'; end if;

  select * into v_previous from public.model_deployments where user_id=p_user_id and status='active' for update;
  if found then
    update public.model_deployments set status='rolled_back',ended_at=now() where id=v_previous.id;
    update public.model_candidates set status='retired',retired_at=now() where id=v_previous.candidate_id and id<>p_candidate_id;
  end if;
  update public.model_candidates set status='production',approved_at=coalesce(approved_at,now()),promoted_at=now(),retired_at=null where id=p_candidate_id;
  ...
  values(p_user_id,p_candidate_id,v_candidate.ollama_model_name,'active',v_previous.id,p_approved_by,p_notes) returning id into v_deployment_id;
...
begin
  if p_approved_by is distinct from p_user_id then raise exception 'Approver must match the model owner'; end if;
  select * into v_target from public.model_deployments where id=p_target_deployment_id and user_id=p_user_id for update;
  if not found then raise exception 'Rollback target not found'; end if;
  if v_target.status not in ('rolled_back','shadow') then raise exception 'Deployment status % is not a rollback target',v_target.status; end if;
  select * into v_active from public.model_deployments where user_id=p_user_id and status='active' for update;
