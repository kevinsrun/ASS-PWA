...
class Query{constructor(){filters={};}select(){return this;}eq(k,v){filters[k]=v;return this;}gte(k,v){filters[`gte_${k}`]=v;return this;}lte(k,v){filters[`lte_${k}`]=v;return this;}order(){return this;}limit(){return this;}then(resolve){return Promise.resolve({data:Array.from({length:rowCount},()=>({task_type:'email_triage',input_text:'Synthetic reference date\nnot a deadline',final_label:'reference',correction_source:'USER_CORRECTION',model_confidence:0.6})),error:null}).then(resolve);}}
...
assert.equal(response.status,200);assert.equal(filters.user_id,'owner');assert.equal(filters.quality,'approved');assert.equal(filters.task_type,'email_triage');assert.equal(filters.correction_source,'USER_CORRECTION');assert.equal(filters.gte_model_confidence,0.5);
const line=JSON.parse((await response.text()).trim());assert.equal(line.input,'Synthetic reference date\nnot a deadline');assert.equal(line.output,'reference');assert(!('user_id' in line));assert(!('context_json' in line));
