...
type Status = {
  ...
  gmailPush?:{configured:boolean;watches:Array<Record<string,unknown>>;queue:Array<Record<string,unknown>>};
};
  ...
  return (
    <main className="now-page">
      ...
      {status ? <>
        ...
        {status.gmailPush ? <section className="now-section"><div className="now-section-title"><span>Gmail push</span></div><p>{status.gmailPush.configured ? "Authenticated Pub/Sub configuration present; watch/delivery health shown below" : "Push is not configured. Gmail is using scheduled reconciliation."}</p>{status.gmailPush.watches.map(watch=><article key={String(watch.google_account_id)}><strong>{String(status.accounts.find(account=>account.id===watch.google_account_id)?.connected_email ?? "Gmail account")}</strong><p>Watch expiration: {when(watch.watch_expiration)} · Renewed: {when(watch.last_watch_renewal)}</p><p>Last notification: {when(watch.last_successful_push)} · Last processed: {when(watch.last_successful_sync)}</p><p>Watch baseline: {String(watch.watch_history_id ?? "None")} · Processed cursor: {String(status.accounts.find(account=>account.id===watch.google_account_id)?.gmail_history_id ?? "None")}</p>{watch.last_error ? <p role="alert">{String(watch.last_error)}</p> : null}<pre style={{whiteSpace:"pre-wrap",overflowWrap:"anywhere"}}>{JSON.stringify(watch.last_metrics,null,2)}</pre></article>)}<details><summary>Recent notification queue</summary><pre style={{whiteSpace:"pre-wrap",overflowWrap:"anywhere"}}>{JSON.stringify(status.gmailPush.queue,null,2)}</pre></details></section> : null}
