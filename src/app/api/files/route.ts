...
function failure(error: unknown) {
  ...
  console.error(JSON.stringify({ service: "file-intelligence", stage: "failed", message }));
  return NextResponse.json({ error: message }, { status });
}

function safeFileName(name: string) {
  return name.normalize("NFKD").replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 160) || "file";
}

function validatedMimeType(file: File) {
  const extension = file.name.split(".").pop()?.toLowerCase() ?? "";
  if (!allowedExtensions.has(extension)) throw new ApiAuthError("That file type is not supported yet.", 415);
  const mime = file.type || (extension === "md" ? "text/markdown" : extension === "csv" ? "text/csv" : extension === "txt" ? "text/plain" : "");
  if (!allowedMimeTypes.has(mime)) throw new ApiAuthError("The file type does not match its extension.", 415);
  return mime;
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!(file instanceof File)) throw new ApiAuthError("Choose a file to import.", 400);
    if (file.size <= 0 || file.size > MAX_BYTES) throw new ApiAuthError("Files must be between 1 byte and 25 MB.", 413);
    ...
    if (!linkedCourseId && analysis.classification === "syllabus" && courseName) {
      const canvasId = 9_000_000_000 + createHash("sha256").update(`${user.id}:${checksum}:course`).digest().readUInt32BE(0);
      const courseCode = String(analysis.structuredData.courseCode ?? courseName.match(/[A-Z]{2,5}\s*\d{3,5}/i)?.[0] ?? "Course").slice(0, 80);
      const { data: course, error: courseError } = await supabase.from("academic_courses").upsert({
        user_id: user.id, canvas_id: canvasId, name: courseName.slice(0, 240), course_code: courseCode,
        instructor_name: String(analysis.structuredData.professor ?? "").slice(0, 240) || null,
        syllabus_html: analysis.summary, raw_data: { source: "file", importedFileId: fileId, ...analysis.structuredData }, updated_at: new Date().toISOString(),
      }, { onConflict: "user_id,canvas_id" }).select("id").single();
      if (courseError || !course) throw courseError ?? new Error("Could not connect this syllabus to Academics");
    ...
    if (extractionError || !extraction) throw extractionError ?? new Error("Could not save file analysis");
    ...
    const autoCommit = insertedItems.filter((item) => item.required && Number(item.confidence) >= 0.94 && ["assignment", "deadline", "task"].includes(item.item_type));
    for (const item of autoCommit) await commitExtractionItem(user.id, item.id);
    const pendingCount = insertedItems.length - autoCommit.length;
    const completedAt = new Date().toISOString();
    const { error: updateError } = await supabase.from("imported_files").update({
      status: pendingCount > 0 ? "needs_review" : "extracted", classification: analysis.classification,
    ...
    await supabase.from("assistant_alerts").upsert({
      ...
      summary: analysis.summary || `Found ${insertedItems.length} extracted items.`,
      recommendation: pendingCount ? `Review ${pendingCount} suggested action${pendingCount === 1 ? "" : "s"} in Inbox.` : "The file is ready.",
      action_type: "open_inbox", action_payload: { importedFileId: fileId }, status: "pending", updated_at: completedAt,
