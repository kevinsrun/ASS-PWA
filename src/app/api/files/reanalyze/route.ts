...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = await request.json() as { fileId?: string; sourceId?: string };
    ...
    let input: { name: string; mimeType: string; buffer: Buffer };
    let version: string;
    if (body.fileId) {
      ...
      const { data: download, error: downloadError } = await db.storage.from(file.storage_bucket ?? "ass-imports").download(file.storage_path);
      if (downloadError || !download) throw downloadError ?? new Error("Source file is unavailable");
      ...
      input = {name:file.name,mimeType:file.mime_type,buffer:Buffer.from(await download.arrayBuffer())};
    } else {
      ...
      if (source.source_type === "google_drive") {
        ...
        const downloaded = await downloadDriveFile(user.id,source.connected_account_id,source.external_id);
        input = {name:downloaded.name,mimeType:downloaded.mimeType,buffer:downloaded.buffer};
      } else {
        if (!source.content) throw new ApiAuthError("Source text is unavailable for re-analysis",409);
        input = {name:String(source.file_metadata?.title ?? "Imported text")+".txt",mimeType:"text/plain",buffer:Buffer.from(source.content)};
      }
    }
    ...
    if (fileId) { const {error} = await db.from("imported_files").update({classification:analysis.classification,status:"needs_review",last_analyzed_at:new Date().toISOString(),processing_error:null}).eq("id",fileId).eq("user_id",user.id); if(error)throw error; }
    ...
    return NextResponse.json({updated:merge.updates.length,added:merge.additions.length,preserved:merge.preserved,requiresReview:true});
  } catch (error) {
    ...
    return NextResponse.json({error:error instanceof Error ? error.message : "Re-analysis failed"},{status:error instanceof ApiAuthError ? error.status : 500});
  }
}
