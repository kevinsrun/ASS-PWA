...
export async function GET(request: NextRequest) {
  try {
    ...
    const [accounts, drive, runs, drafts, actions, extractions] = await Promise.all([
    ...
    const error = [accounts, drive, runs, drafts, actions, extractions].find((result) => result.error)?.error;
