...
export async function GET(request: NextRequest) {
  try {
    ...
    const [accounts, drive, runs, drafts, actions, extractions, assistantRuns, watches, gmailQueue] = await Promise.all([
      ...
      supabase.from("gmail_watch_state").select("google_account_id,watch_expiration,watch_history_id,last_watch_renewal,last_successful_push,last_successful_sync,last_error,last_metrics,lease_until").eq("user_id",user.id),
      supabase.from("gmail_processing_queue").select("id,google_account_id,history_id,status,attempts,next_attempt_at,last_error,created_at,completed_at").eq("user_id",user.id).order("created_at",{ascending:false}).limit(20),
    ]);
    const error = [accounts, drive, runs, drafts, actions, extractions, assistantRuns,watches,gmailQueue].find((result) => result.error)?.error;
    ...
    return NextResponse.json({
      ...
      gmailPush:{configured:gmailPushConfiguration().ready,watches:watches.data ?? [],queue:gmailQueue.data ?? []},
    });
  } catch (error) {
