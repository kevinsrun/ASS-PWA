...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        accounts: await verifyGoogleServices(user.id),
        plaidConfigured: plaidConfiguration().ready,
      },
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !body.accountId ||
      !["verify", "disconnect", "process"].includes(body.action)
    )
      throw new ApiAuthError("Choose an account and action", 400);
    if (body.action === "process") {
      const verified = await verifyGoogleServices(
        user.id,
        body.accountId,
        true,
      );
      if (verified[0]?.gmail.state !== "connected")
        throw new ApiAuthError(
          verified[0]?.gmail.error ?? "Verify Gmail first",
          409,
        );
      return NextResponse.json(
        await scanRecentGmailSuggestions(user.id, body.accountId, undefined, 5),
      );
    }
    if (body.action === "verify")
      return NextResponse.json({
        accounts: await verifyGoogleServices(user.id, body.accountId, true),
      });
    ...
    if (!data)
      throw new ApiAuthError("Connected Google account not found", 404);
