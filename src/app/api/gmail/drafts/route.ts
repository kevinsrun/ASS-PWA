...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === "ignore") { const { error: ignoreError } = await supabase.from("email_drafts").update({ status: "no_response_needed", updated_at: new Date().toISOString() }).eq("id", body.id).eq("user_id", user.id); if (ignoreError) throw ignoreError; return NextResponse.json({ ok: true }); }
