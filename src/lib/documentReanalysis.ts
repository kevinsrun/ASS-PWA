...
export async function reanalyzeSource(userId: string, body: {fileId?: string;sourceId?: string}) {
  let lease: {table:string;id:string;userId:string;field:string;previous:string} | null = null;
  let leaseClaimed = false;
  try {
    const user = {id:userId};
    if (Boolean(body.fileId) === Boolean(body.sourceId)) throw new ApiAuthError("Choose one imported file or text source",400);
    const db = getServiceSupabaseClient(); if (!db) throw new Error("A Supabase server key is not configured");
    let fileId: string | null = null, sourceId: string | null = body.sourceId ?? null;
    ...
    if (body.fileId) {
      ...
      fetchOriginal=async()=>{const {data:download,error:downloadError}=await db.storage.from(file.storage_bucket ?? "ass-imports").download(file.storage_path);if(downloadError||!download)throw downloadError ?? new Error("Source file is unavailable");return {name:file.name,mimeType:file.mime_type,buffer:Buffer.from(await download.arrayBuffer())};};
    } else {
      ...
      if (source.source_type === "google_drive") {
        if (!source.connected_account_id) throw new ApiAuthError("Reconnect this Google Drive account before re-analysis",409);
        fetchOriginal=async()=>{const downloaded=await downloadDriveFile(user.id,source.connected_account_id,source.external_id);return {name:downloaded.name,mimeType:downloaded.mimeType,buffer:downloaded.buffer};};
      } else {
        if (!source.content) throw new ApiAuthError("Source text is unavailable for re-analysis",409);
    ...
    query = fileId ? query.eq("imported_file_id",fileId) : query.eq("imported_source_id",sourceId);
    ...
    for (const update of merge.updates) {
      const {error} = await db.from("extraction_items").update(row(update.item)).eq("user_id",user.id).eq("id",update.id).eq("review_status","pending").is("manual_corrected_at",null).is("deleted_at",null).eq("ignore_future_imports",false);
      if (error) throw error;
    }
    if (merge.additions.length) { const {error} = await db.from("extraction_items").insert(merge.additions.map(item=>({...row(item),user_id:user.id,imported_file_id:fileId,imported_source_id:sourceId}))); if(error)throw error; }
    if (merge.unmatched.length) {
      const {error} = await db.from("extraction_items").update({item_type:"unknown",normalized_type:"reference",confidence:0.49,classification_reason:"Not reproduced by the improved factual analysis. No action was created.",updated_at:new Date().toISOString()}).eq("user_id",user.id).in("id",merge.unmatched).eq("review_status","pending").is("manual_corrected_at",null).is("deleted_at",null).eq("ignore_future_imports",false);
      if(error)throw error;
    }
    if (fileId) { const {error} = await db.from("imported_files").update({classification:analysis.classification,status:"needs_review",last_analyzed_at:new Date().toISOString(),processing_error:null,updated_at:new Date().toISOString()}).eq("id",fileId).eq("user_id",user.id); if(error)throw error; }
    if (sourceId) { const {error} = await db.from("imported_sources").update({processing_status:"needs_review",processing_error:null,updated_at:new Date().toISOString()}).eq("id",sourceId).eq("user_id",user.id); if(error)throw error; }
    // Re-analysis updates suggestions only. Never duplicates or mutates existing
    // calendar/tasks, manual corrections, accepted decisions, or ignored records.
    return {updated:merge.updates.length,added:merge.additions.length,preserved:merge.preserved,requiresReview:true,changes:merge.updates.map(update=>{const previous=(existing ?? []).find(item=>item.id===update.id);return {id:update.id,title:update.item.title,from:previous?.normalized_type,to:update.item.normalizedType,timeChanged:previous?.due_at!==update.item.dueAt};}),removedFalseEvents:(existing ?? []).filter(item=>merge.unmatched.includes(item.id)&&item.normalized_type==="calendar_event").length};
  } catch (error) {
