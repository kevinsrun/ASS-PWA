...
export async function ingestDriveFile(userId: string, accountId: string, fileId: string, selectedByUser = false) {
  const supabase = getServiceSupabaseClient();
  if (!supabase) throw new Error("A Supabase server key is not configured");
  const { data: existing } = await supabase.from("imported_sources").select("id,processing_status").eq("user_id", userId).eq("source_type", "google_drive").eq("connected_account_id", accountId).eq("external_id", fileId).maybeSingle();
  if (existing) return { id: String(existing.id), itemCount: 0, pendingCount: 0, alreadyImported: true };
  ...
  try {
    ...
    const { data: source, error: sourceError } = await supabase.from("imported_sources").insert({
      user_id: userId, source_type: "google_drive", connected_account_id: accountId, external_id: fileId,
    ...
    for (const item of autoCommit) await commitExtractionItem(userId, item.id);
    ...
    if (writingText) await classifyWritingSpans(userId, sourceId, writingText, "drive");
    ...
    console.info(JSON.stringify({ service: "drive-intelligence", stage: "completed", sourceId, driveFileId: fileId, items: items.length, autoCommitted: autoCommit.length }));
    return { id: sourceId, itemCount: items.length, pendingCount, alreadyImported: false };
  } catch (error) {
    const message = error instanceof Error ? error.message : "Drive import failed.";
    if (sourceId) await supabase.from("imported_sources").update({ processing_status: "failed", processing_error: message.slice(0, 2000), updated_at: new Date().toISOString() }).eq("id", sourceId);
    console.error(JSON.stringify({ service: "drive-intelligence", stage: "failed", sourceId, fileId, message }));
    throw error;
  }
}
